    ...
    if(isset($_POST['lokata'])) {
        ...
        try {
            ...
            if($polaczenie->connect_errno!=0) {
                ...
                $wynik = $polaczenie->query("CALL SprawdzKwote(".$_SESSION['id'].",$lokata)");

                if(!$wynik) throw new Exception($polaczenie->error);

                $licz_lokate = $wynik->num_rows;
                if($licz_lokate == 0) {
                  $poprawnosc = false;
                  $_SESSION['e_lokata'] = 'Nie masz wystarczającej ilości środków na koncie!';
                }
                $polaczenie->close();
                if($poprawnosc == true) {
                  try {
                      ...
                      if($polaczenie->connect_errno!=0) {
                          ...
                          if($polaczenie->query("CALL WplacLokata(".$_SESSION['id'].",$lokata)")) {
                              $_SESSION['udanalokata'] = true;
                              header('Location: ulokata.php');
                          } else {
...
<html>
    ...
    <body>
        ...
        <!-- WYSUWANE OKNA -->
            ...
            <section class="bank-deposit wrapper" style="display:block">
                <table>
                   <tr>
                       <th>Stan lokaty&#58;</th>
                   </tr>
               ...
               <form method="POST" action="deposit.php">
                  ...
                  <?php
                      if(isset($_SESSION['e_lokata'])) {
                          echo '<p class="error">'.$_SESSION['e_lokata'].'</p>';
                          unset($_SESSION['e_lokata']);
                      }
                  ?>
